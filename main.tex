...
\appendix
\chapter{実験の詳細}
表A.1および表A.2はKorfの作成した100問の15puzzleの問題集\cite{Kor85}を解いた各種アルゴリズムの実行時間(sec)である。
表A.3は性能評価のために用いた自己生成した24-puzzleの問題集の詳細である。
問題集についてはパズルの初期状態、最短経路、マンハッタン距離もしくはPDBをヒューリスティック関数に使用した際にIDA*探索が、全ての最短経路を見つけるのに必要な展開ノードの数について表記する。
