...
A*探索の問題点として、メモリ使用量の大きさがあげられる。これは探索で生成した全ての状態をメモリに保存するため、状態空間の大きな問題では、メモリを使いきってしまうためである。このような問題点を解決する手法として、後述のIDA*探索\cite{Kor85}、メモリが不足するとf値の大きなノードをオープンリストから消去して探索を行うSMA*探索\cite{Rus92}などがある。
...
\subsection{IDA*探索}
IDA*探索\cite{Kor85}はKorfによって提案された手法であり、A*探索同様ヒューリスティック関数を用いたアルゴリズムである。

そのアルゴリズムは反復深化深さ優先探索にA*探索を応用したものである。

IDA*探索では、深さを制限した深さ制限探索に、f値による枝刈りを加えたf値制限深さ優先探索である。

その特徴として、メモリ使用量があげられる。A*が探索空間に比例した空間計算量を使用したのに対し、IDA*探索では、探索の深さに対して線形を保つことができるため、A*探索では、メモリ不足で解けない問題においても解を発見することが可能である。

空間計算量がの数と比例するA*探索に対し、IDA*探索では探索の深さに対して線形を保つことができる。IDA*ではA*と同様にヒューリスティック関数によってyotte状態の評価を行い、展開の順序を決定するアルゴリズムである。またIDA*探索で用いるヒューリスティック関数を用いず
