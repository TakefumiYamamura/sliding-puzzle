...
\section{先行研究}
堀江らの先行研究\cite{Horie17}では、GPUを使ったスレッドベースの並列化を行うParallel IDA*探索とブロックベースのBlock Parallel IDA*探索が提唱された。
本節では、この二つ手法についてそれぞれ説明する。

\subsection{Parallel IDA*}
堀江らの研究\cite{Horie17}では、スレッドベースの並列化を行うParallel IDA*探索が
