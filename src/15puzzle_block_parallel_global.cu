...
__global__ void dfs_kernel(int limit, Node *root_set, int *dev_flag, Lock *lock, int *loop_set, Node *global_st) {
...
__global__ void dfs_kernel(int limit, Node *root_set, int *dev_flag, int *loop_set, Node *global_st) {
    ...
    // __shared__ Node global_st[STACK_LIMIT];
...
void ida_star() {
    ...
    for (int limit = s_node.md; limit < 100; ++limit, ++limit)
    {
        ...
        #ifdef USE_LOCK
        dfs_kernel<<<root_node_size, THREAD_SIZE_PER_BLOCK>>>(limit, dev_root_set, dev_flag, dev_lock, dev_load_set, global_st);
        #else
